...
module Roxanne
  class YAMLConfigurationTest < Test::Unit::TestCase
    ...
    def test_initialize
      config = Roxanne::Configuration::YAMLConfiguration.new(Dir.getwd, 'config/test.yml')
      assert_equal 1, config.consumers.length, 'Configuration should contain a consumer'
      assert_equal Roxanne::Jenkins::JenkinsConsumer, config.consumers[0].class, 'First consumer should be an hudson consumer'
